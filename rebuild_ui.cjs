...
if (!txt.includes('lucide-react')) {
  ...
  // If it is there we might need to add our specific icons
...
// 2. Replace the main middle block (the old "驾校宝典" circular layout) and the Theme Grid
const newMiddleBlock = `
      ...
      <div style={{ margin: '0 4px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          <Star size={16} color="#FFD5A0" fill="#FFD5A0" />
          <span style={{ fontSize: 16, fontWeight: 900, color: '#f5efe8', letterSpacing: 0.5 }}>按主题精练</span>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {(Object.keys(CATEGORY_META) as QuizCategory[]).map(cat => {
            ...
            return (
              ...
              >
                {/* 装饰色条 */}
                <div style={{ position: 'absolute', left: 0, top: '20%', bottom: '20%', width: 3, background: count > 0 ? meta.color : 'transparent', borderRadius: '0 4px 4px 0', opacity: 0.6 }} />
                
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  {/* 使用抽象化文字/色彩气泡代替丑陋大Emoji */}
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: \`\${meta.color}15\`, color: meta.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>
                    {meta.emoji}
                  </div>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      <span style={{ fontSize: 16, fontWeight: 800, color: count > 0 ? '#f5efe8' : 'rgba(245,239,232,0.3)' }}>{meta.label}</span>
                      {!hasMicroVip && allCount > freeCount && (
                        <span style={{
                          fontSize: 9, color: '#3a2c17', fontWeight: 900,
                          background: 'linear-gradient(90deg, #FFD5A0, #FFBD73)', borderRadius: 4, padding: '2px 6px',
                        }}>VIP</span>
                      )}
                    </div>
                    <div style={{ fontSize: 12, color: 'rgba(245,239,232,0.45)' }}>{meta.desc}</div>
                  </div>
                </div>
                
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: 'rgba(245,239,232,0.6)' }}>
...
const endBound = "        <div style={{ marginTop: 24 }}>";
...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  // If can't find bounds, write to a debug file and print what it couldn't find
