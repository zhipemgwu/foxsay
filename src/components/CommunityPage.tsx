...
import { IconBubble, IcFire, IcChat, IcHeartSpark, IcMask, IcTrophy, IcPen, IcSparkle, IcHeart as IcHeartIcon, IcShield, IcBook, IcStar, gradients } from './CuteIcons';
...
const emotionTags = [
  { label: '暗恋中', emoji: '💗', bg: 'rgba(255,138,128,0.18)', color: '#FF8A80' },
  { label: '被拒绝了', emoji: '💔', bg: 'rgba(155,126,222,0.18)', color: '#9B7EDE' },
  { label: '不知道怎么开口', emoji: '😶', bg: 'rgba(100,181,246,0.15)', color: '#64B5F6' },
  { label: '刚分手', emoji: '🌧️', bg: 'rgba(206,147,216,0.18)', color: '#CE93D8' },
  { label: '恋爱甜蜜', emoji: '🍯', bg: 'rgba(255,217,61,0.18)', color: '#FFD93D' },
  { label: '求助', emoji: '🆘', bg: 'rgba(78,205,196,0.18)', color: '#4ECDC4' },
];

const initialTreeHolePosts = [
  { id: 1, animal: 0, emotion: '暗恋中', content: '同事坐在我对面，每天都能看到他认真工作的侧脸。他笑的时候我就心跳加速，但是我不敢说...', time: '1小时前', hugs: 23, cheers: 15, feels: 42, myReaction: null },
  { id: 2, animal: 3, emotion: '被拒绝了', content: '鼓起勇气表白了，结果他说"你是个好人"。回家哭了一晚上，但我不后悔。至少我勇敢过。', time: '3小时前', hugs: 67, cheers: 34, feels: 89, myReaction: null },
  { id: 3, animal: 5, emotion: '不知道怎么开口', content: '认识三年了，从朋友变成了我心里最特别的人。每次想说喜欢都卡在嗓子眼...该怎么开口？', time: '5小时前', hugs: 45, cheers: 28, feels: 56, myReaction: null },
  { id: 4, animal: 2, emotion: '恋爱甜蜜', content: '今天他偷偷在我包里塞了一颗糖，附带一张小纸条写着"想你了"。这种小浪漫真的好甜 🥰', time: '昨天', hugs: 12, cheers: 88, feels: 34, myReaction: null },
  { id: 5, animal: 7, emotion: '刚分手', content: '分手第15天。路过我们常去的奶茶店，鬼使神差点了他爱喝的那杯。喝着喝着眼泪就掉下来了。', time: '昨天', hugs: 89, cheers: 23, feels: 67, myReaction: null },
  { id: 6, animal: 4, emotion: '求助', content: '异地恋第二年了，我越来越觉得孤单。他说没事的再忍忍，可是我真的好累。有过来人给点建议吗？', time: '2天前', hugs: 78, cheers: 45, feels: 56, myReaction: null },
  { id: 7, animal: 1, emotion: '暗恋中', content: '他今天回了我一个"哈哈哈"而不是"哈哈"。我是不是可以开心一下？（我真的好卑微）', time: '2天前', hugs: 34, cheers: 56, feels: 78, myReaction: null },
  { id: 8, animal: 6, emotion: '不知道怎么开口', content: '毕业季最后一个月了。再不说，可能这辈子都不会说了。但万一被拒绝，连朋友都做不了...', time: '3天前', hugs: 56, cheers: 67, feels: 45, myReaction: null },
];
...
export function CommunityPage() {
  ...
  const [treeHolePosts, setTreeHolePosts] = useState(initialTreeHolePosts);
  const [showTreeHoleCompose, setShowTreeHoleCompose] = useState(false);
  const [treeHoleComposeText, setTreeHoleComposeText] = useState('');
  const [treeHoleEmotion, setTreeHoleEmotion] = useState('');
  const [treeHoleFilter, setTreeHoleFilter] = useState('全部');

  const toggleReaction = (postId, type) => {
    setTreeHolePosts(prev => prev.map(p => {
      if (p.id !== postId) return p;
      const wasThis = p.myReaction === type;
      const updated = { ...p, myReaction: wasThis ? null : type };
      if (p.myReaction && p.myReaction !== type) updated[p.myReaction] = p[p.myReaction] - 1;
      updated[type] = wasThis ? p[type] - 1 : p[type] + 1;
      return updated;
    }));
  };

  const filteredTreeHole = treeHolePosts.filter(p => treeHoleFilter === '全部' || p.emotion === treeHoleFilter);
  ...
  if (readingArticle) {
    ...
    const isUnlocked = !art.isPaid || (user.isPro?.() ?? user.isVip) || unlockedArticles.has(art.id);
    ...
    return (
      <div className="px-5 pt-6 pb-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          <div className="cursor-pointer" onClick={(e) => { e.stopPropagation(); openProfile({ name: art.author, avatarIdx: art.authorIdx }); }}>
          ...
          {art.isPaid && !isUnlocked && (
            <span className="flex items-center gap-1 px-2 py-1" style={{ background: 'rgba(255,217,61,0.18)', borderRadius: 6 }}>
              ...
              <span style={{ color: '#FFD93D', fontSize: '11px', fontWeight: 600 }}>VIP</span>
            </span>
          )}
          {art.isPaid && isUnlocked && (
            ...
            <motion.div className="p-[1px] mb-6" style={{ borderRadius: 16, background: 'linear-gradient(135deg, rgba(255,217,61,0.4), rgba(255,138,128,0.2), rgba(155,126,222,0.2))' }}
              ...
              <div className="p-6 flex flex-col items-center" style={{ background: '#453a60', borderRadius: 15 }}>
                ...
                <motion.button className="w-full py-3.5 flex items-center justify-center gap-2"
                  ...
                  whileTap={{ scale: 0.98 }}
                  aria-label="开通会员解锁全文"
                  onClick={() => sub.open('community_article')}>
                  <Lock size={14} /> 开通会员解锁全文
                </motion.button>
                <p style={{ color: 'rgba(245,239,232,0.35)', fontSize: '11px', marginTop: 8 }}>会员尊享全部专栏内容</p>
              </div>
            </motion.div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between pt-4" style={{ borderTop: '1px solid rgba(245,239,232,0.10)' }}>
          <div className="flex items-center gap-5">
            <motion.button className="flex items-center gap-1.5" whileTap={{ scale: 0.95 }}>
      ...
      <>
        <div className="px-5 pt-6 pb-8 relative overflow-hidden">
          ...
          <div className="absolute top-0 left-0 right-0 pointer-events-none" style={{
            height: 240,
            background: 'radial-gradient(ellipse 80% 50% at 50% 0%, rgba(155,126,222,0.20) 0%, rgba(78,205,196,0.08) 40%, transparent 100%)',
          }} />

          {/* 返回 + 标题 */}
          <motion.button className="flex items-center gap-1 mb-4 relative z-10" onClick={() => setShowTreeHole(false)}
            whileTap={{ scale: 0.95 }} style={{ color: 'rgba(245,239,232,0.65)', fontSize: '14px' }}>
            <ChevronLeft size={18} /> 返回讨论
          </motion.button>

          <div className="flex items-center gap-3 mb-5 relative z-10">
            <IconBubble size={42} bg={gradients.purple}><IcMask size={18} color="#fff" /></IconBubble>
            <div>
              <h1 style={{ color: '#f5efe8', fontSize: '24px', fontWeight: 700, margin: 0 }}>匿名树洞</h1>
              <p style={{ color: 'rgba(245,239,232,0.55)', fontSize: '13px', marginTop: 2 }}>说出你不敢说的心里话 · 所有人匿名</p>
            </div>
          </div>

          {/* 温馨提示 */}
          <motion.div className="p-3.5 mb-4 flex items-center gap-3"
            style={{ background: 'rgba(78,205,196,0.08)', borderRadius: 12, border: '1px solid rgba(78,205,196,0.12)' }}
            initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
            <span style={{ fontSize: 20 }}>🤫</span>
            <p style={{ color: 'rgba(245,239,232,0.6)', fontSize: '12px', lineHeight: 1.5 }}>
              这里所有发言都是匿名的，你会被随机分配一个可爱的动物身份。请温柔以待每一份真心。
            </p>
          </motion.div>

          {/* 情绪筛选标签 */}
          <div className="flex gap-2 mb-5 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
            <button onClick={() => setTreeHoleFilter('全部')} className="flex-shrink-0 px-3 py-1.5"
              style={{ background: treeHoleFilter === '全部' ? '#9B7EDE' : '#453a60', borderRadius: 980, color: treeHoleFilter === '全部' ? '#fff' : 'rgba(245,239,232,0.55)', fontSize: '12px', fontWeight: treeHoleFilter === '全部' ? 600 : 400 }}>
              全部
            </button>
            {emotionTags.map(t => (
              <button key={t.label} onClick={() => setTreeHoleFilter(t.label)} className="flex-shrink-0 flex items-center gap-1 px-3 py-1.5"
                style={{ background: treeHoleFilter === t.label ? t.color : '#453a60', borderRadius: 980, color: treeHoleFilter === t.label ? '#fff' : 'rgba(245,239,232,0.55)', fontSize: '12px', fontWeight: treeHoleFilter === t.label ? 600 : 400 }}>
          ...
          <div className="flex flex-col gap-3">
            {filteredTreeHole.length === 0 ? (
              <motion.div className="flex flex-col items-center py-12" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <span style={{ fontSize: 48, marginBottom: 12 }}>🌙</span>
                <p style={{ color: 'rgba(245,239,232,0.55)', fontSize: '14px' }}>这个情绪下还没有倾诉</p>
                <p style={{ color: 'rgba(245,239,232,0.4)', fontSize: '12px', marginTop: 4 }}>成为第一个说出心里话的人吧</p>
              </motion.div>
            ) : (
              filteredTreeHole.map((post, idx) => {
                const animal = anonymousAnimals[post.animal];
                const emTag = emotionTags.find(e => e.label === post.emotion) || emotionTags[0];
                return (
                  <motion.div key={post.id}
                    className="p-[1px]"
                    style={{ borderRadius: 16, background: 'linear-gradient(135deg, rgba(155,126,222,0.12), transparent, transparent)' }}
                    initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: idx * 0.05 }}>
                    <div className="p-5" style={{ background: '#453a60', borderRadius: 15 }}>
                      {/* 匿名头像 + 情绪标签 */}
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <div className="flex items-center justify-center"
                            style={{ width: 36, height: 36, borderRadius: 18, background: 'rgba(155,126,222,0.15)', fontSize: 18 }}>
                            {animal.emoji}
                          </div>
                          <div>
                            <span style={{ color: '#f5efe8', fontSize: '13px', fontWeight: 600 }}>{animal.name}</span>
                            <p style={{ color: 'rgba(245,239,232,0.4)', fontSize: '11px' }}>{post.time}</p>
                          </div>
                        </div>
                        <span className="flex items-center gap-1 px-2 py-1"
                          style={{ background: emTag.bg, borderRadius: 6 }}>
                          <span style={{ fontSize: 10 }}>{emTag.emoji}</span>
                          <span style={{ color: emTag.color, fontSize: '10px', fontWeight: 600 }}>{post.emotion}</span>
                        </span>
                      </div>

                      {/* 内容 */}
                      <p style={{ color: 'rgba(245,239,232,0.82)', fontSize: '14px', lineHeight: 1.7, marginBottom: 14, whiteSpace: 'pre-line' }}>
                        {post.content}
                      </p>
                      ...
                      <div className="flex items-center gap-2">
                        {([
                          { key: 'hugs', emoji: '🤗', label: '拥抱' },
                          { key: 'cheers', emoji: '💪', label: '加油' },
                          { key: 'feels', emoji: '💕', label: '懂你' },
                        ]).map(r => (
                          <motion.button key={r.key}
                            className="flex items-center gap-1.5 px-3 py-1.5"
                            style={{
                              background: post.myReaction === r.key ? 'rgba(155,126,222,0.2)' : 'rgba(245,239,232,0.05)',
                              borderRadius: 980,
                              border: post.myReaction === r.key ? '1px solid rgba(155,126,222,0.3)' : '1px solid rgba(245,239,232,0.08)',
                            }}
                            whileTap={{ scale: 0.92 }}
                            onClick={() => toggleReaction(post.id, r.key)}>
                            <span style={{ fontSize: 12 }}>{r.emoji}</span>
                            <span style={{ color: post.myReaction === r.key ? '#9B7EDE' : 'rgba(245,239,232,0.45)', fontSize: '11px', fontWeight: 500 }}>
                              {r.label} {post[r.key]}
                            </span>
                          </motion.button>
                        ))}
        ...
        {/* 匿名发布按钮 */}
        <motion.button
          className="fixed z-[50] flex items-center gap-2 px-5 py-3.5"
          style={{
            bottom: 90, right: 'calc(50% - 195px)',
            background: gradients.purple,
            borderRadius: 980, boxShadow: '0 8px 30px rgba(155,126,222,0.4)',
          }}
          whileTap={{ scale: 0.95 }}
          onClick={() => setShowTreeHoleCompose(true)}
          initial={{ opacity: 0, y: 20, scale: 0.9 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ delay: 0.3 }}>
          <IcPen size={14} color="#fff" />
          <span style={{ color: '#fff', fontSize: '14px', fontWeight: 600 }}>匿名倾诉</span>
        </motion.button>
          ...
          {showTreeHoleCompose && (
            <motion.div className="fixed inset-0 z-[55] flex flex-col" style={{ background: '#2b2535' }}
              ...
              <div style={{ paddingTop: 'env(safe-area-inset-top, 44px)' }}>
                <div className="flex items-center justify-between px-5 h-14">
                  <motion.button whileTap={{ scale: 0.9 }} onClick={() => {
                    if (treeHoleComposeText.trim() && !window.confirm('放弃编辑？已输入的内容将不会保存')) return;
                    setTreeHoleComposeText(''); setTreeHoleEmotion(''); setShowTreeHoleCompose(false);
                  }} style={{ color: 'rgba(245,239,232,0.65)', fontSize: '14px' }}>取消</motion.button>
                  <div className="flex items-center gap-2">
                    <span style={{ fontSize: 16 }}>🕳️</span>
                    <span style={{ color: '#f5efe8', fontSize: '16px', fontWeight: 600 }}>匿名倾诉</span>
                  </div>
                  <motion.button className="px-4 py-1.5"
                    style={{
                      background: (treeHoleComposeText.trim() && treeHoleEmotion) ? gradients.purple : '#453a60',
                      borderRadius: 980,
                      color: (treeHoleComposeText.trim() && treeHoleEmotion) ? '#fff' : 'rgba(245,239,232,0.38)',
                      fontSize: '14px', fontWeight: 600,
                    }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => {
                      if (treeHoleComposeText.trim() && treeHoleEmotion) {
                        const randomAnimal = Math.floor(Math.random() * anonymousAnimals.length);
                        setTreeHolePosts(prev => [{
                          id: Date.now(), animal: randomAnimal, emotion: treeHoleEmotion,
                          content: treeHoleComposeText, time: '刚刚',
                          hugs: 0, cheers: 0, feels: 0, myReaction: null,
                        }, ...prev]);
                        setTreeHoleComposeText(''); setTreeHoleEmotion(''); setShowTreeHoleCompose(false);
                      }
                    }}>
              ...
              <div className="flex-1 px-5 py-4 overflow-y-auto">
                {/* 匿名身份提示 */}
                <div className="flex items-center gap-2 mb-4 p-3" style={{ background: 'rgba(155,126,222,0.08)', borderRadius: 10 }}>
                  <span style={{ fontSize: 14 }}>🎭</span>
                  <span style={{ color: 'rgba(245,239,232,0.6)', fontSize: '12px' }}>你将以随机动物身份匿名发布，没有人知道你是谁</span>
                </div>

                {/* 选择情绪 */}
                <p style={{ color: 'rgba(245,239,232,0.65)', fontSize: '13px', fontWeight: 600, marginBottom: 10 }}>你现在的心情</p>
                <div className="flex flex-wrap gap-2 mb-5">
                  {emotionTags.map(t => (
                    <motion.button key={t.label}
                      className="flex items-center gap-1.5 px-3 py-2"
                      style={{
                        background: treeHoleEmotion === t.label ? t.bg : '#453a60',
                        borderRadius: 10,
                        border: treeHoleEmotion === t.label ? `1px solid ${t.color}` : '1px solid rgba(245,239,232,0.08)',
                      }}
                      whileTap={{ scale: 0.95 }}
                      onClick={() => setTreeHoleEmotion(treeHoleEmotion === t.label ? '' : t.label)}>
                      <span style={{ fontSize: 13 }}>{t.emoji}</span>
                      <span style={{ color: treeHoleEmotion === t.label ? t.color : 'rgba(245,239,232,0.55)', fontSize: '13px', fontWeight: treeHoleEmotion === t.label ? 600 : 400 }}>
                        {t.label}
                      </span>
                    </motion.button>
                  ))}
                ...
                {/* 输入内容 */}
                <textarea autoFocus value={treeHoleComposeText} onChange={e => setTreeHoleComposeText(e.target.value)}
                  placeholder="在这里写下你的心里话...&#10;&#10;不用担心，这里没有人认识你 💫"
                  className="w-full h-48 bg-transparent outline-none resize-none"
                  style={{ color: '#f5efe8', fontSize: '15px', lineHeight: 1.7 }} />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-1 p-1 mb-5 relative" style={{ background: '#453a60', borderRadius: 12 }}>
          {[{ key: 'column', label: '专栏' }, { key: 'discuss', label: '讨论' }].map((tab, i) => (
            <button key={tab.key} onClick={() => setMainTab(tab.key)}
              ...
              style={{
                background: mainTab === tab.key ? '#453a60' : 'transparent',
                borderRadius: 10,
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }}>
            ...
            <div className="mb-5">
              ...
              <div className="flex gap-3 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
                {bloggers.map((b, bIdx) => (
                  <motion.div key={b.id} className="flex-shrink-0 flex flex-col items-center p-3 cursor-pointer" style={{ background: '#453a60', borderRadius: 14, width: 100 }}
                    whileTap={{ scale: 0.98 }}
                    onClick={() => openProfile({ name: b.name, avatarIdx: b.avatarIdx })}>
                        ...
                        <div className="flex items-center gap-2">
                          {art.isPaid ? (
                            <span className="flex items-center gap-1 px-1.5 py-0.5" style={{ background: 'rgba(255,217,61,0.18)', borderRadius: 4 }}>
                              ...
                              <span style={{ color: '#FFD93D', fontSize: '10px', fontWeight: 600 }}>VIP</span>
                            </span>
                          ) : (
            ...
            <div className="flex gap-2 overflow-x-auto mb-4" style={{ scrollbarWidth: 'none' }}>
              {[
                ...
                <motion.button key={t.label} className="flex-shrink-0 flex items-center gap-2 px-3 py-2.5"
                  style={{ background: '#453a60', borderRadius: 12 }} whileTap={{ scale: 0.98 }}>
                  ...
                  return (
                    <motion.div key={post.id} className="p-[1px]"
                      ...
                      <div className="p-5" style={{ background: '#453a60', borderRadius: 15 }}>
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-3">
                            <div className="cursor-pointer" onClick={(e) => { e.stopPropagation(); openProfile({ name: post.user, avatarIdx: post.avatarIdx, level: post.level }); }}>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-5">
                            <motion.button className="flex items-center gap-1.5" whileTap={{ scale: 0.95 }} onClick={() => toggleLike(post.id)}>
                            ...
                            <motion.button className="flex items-center gap-1.5" whileTap={{ scale: 0.95 }} onClick={() => setShowComments(post.id)}>
                          ...
                          <motion.button whileTap={{ scale: 0.95 }} onClick={() => toggleSave(post.id)}>
          ...
          <motion.div className="fixed inset-0 z-[55] flex flex-col" style={{ background: '#2b2535' }}
            ...
            <div style={{ paddingTop: 'env(safe-area-inset-top, 44px)' }}>
              <div className="flex items-center justify-between px-5 h-14">
                ...
                <motion.button className="px-4 py-1.5"
                  style={{ background: composeText.trim() ? gradients.coral : '#453a60', borderRadius: 980, color: composeText.trim() ? '#fff' : 'rgba(245,239,232,0.38)', fontSize: '14px', fontWeight: 600 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={() => {
                    if (composeText.trim()) {
            ...
            <motion.div className="relative mt-auto w-full overflow-hidden"
              ...
              <div className="px-5 py-4 overflow-y-auto" style={{ maxHeight: 'calc(65vh - 120px)' }}>
                {[
                  ...
                  <div key={i} className="flex gap-3 mb-4">
                    <div className="cursor-pointer" onClick={(e) => { e.stopPropagation(); openProfile({ name: c.user, avatarIdx: c.avatarIdx }); }}>
              ...
              <div className="flex items-center gap-3 px-5 py-3" style={{ borderTop: '1px solid rgba(245,239,232,0.10)', paddingBottom: 'env(safe-area-inset-bottom, 12px)' }}>
                <input value={commentInput} onChange={e => setCommentInput(e.target.value)} placeholder="说点什么..."
                  ...
                  style={{ background: '#453a60', borderRadius: 20, color: '#f5efe8', fontSize: '13px' }} />
                <motion.button className="w-9 h-9 rounded-full flex items-center justify-center"
                  style={{ background: commentInput.trim() ? gradients.coral : '#453a60' }} whileTap={{ scale: 0.9 }}>
      ...
      <AnimatePresence>
        {showCoachDetail && (
          <motion.div className="fixed inset-0 z-[1200] flex items-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <div className="p-6">
                ...
                <div className="flex flex-col items-center mb-5">
                  <div className="relative mb-3">
                    <div className="w-20 h-20 rounded-full overflow-hidden" style={{ border: '3px solid rgba(155,126,222,0.5)' }}>
                      <img src={avatarImages[showCoachDetail.avatarIdx]} alt={showCoachDetail.name} className="w-full h-full object-cover" />
                    </div>
                    <div className="absolute -bottom-1 -right-1">
                      <CertifiedBadge size={22} />
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-center gap-6 mb-5">
                  <div className="text-center">
                    <div className="flex items-center gap-1 justify-center">
                      ...
                      <span style={{ color: '#FFD93D', fontSize: 18, fontWeight: 800 }}>4.9</span>
                    </div>
                    <span style={{ color: 'rgba(245,239,232,0.4)', fontSize: 10 }}>评分</span>
                  </div>
                  <div className="w-px h-8" style={{ background: 'rgba(245,239,232,0.08)' }} />
                  <div className="text-center">
                    <span style={{ color: '#f5efe8', fontSize: 18, fontWeight: 800 }}>{showCoachDetail.articles}</span>
                    <p style={{ color: 'rgba(245,239,232,0.4)', fontSize: 10 }}>专栏文章</p>
                  </div>
                  <div className="w-px h-8" style={{ background: 'rgba(245,239,232,0.08)' }} />
                  <div className="text-center">
                    <span style={{ color: '#f5efe8', fontSize: 18, fontWeight: 800 }}>{showCoachDetail.followers}</span>
                ...
                <div className="mb-5 p-3" style={{ background: 'rgba(245,239,232,0.04)', borderRadius: 10 }}>
                  ...
                  <p style={{ color: 'rgba(245,239,232,0.7)', fontSize: 13, lineHeight: 1.6 }}>
                    {showCoachDetail.desc}，拥有多年情感咨询经验。已发布{showCoachDetail.articles}篇专栏文章，{showCoachDetail.followers}粉丝关注。一对一指导风格温和耐心，善于发现学员的潜力。
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <motion.button className="w-full py-3.5 flex items-center justify-center gap-2"
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={() => { const coachId = showCoachDetail.avatarIdx + 1; const t = bookCoach(coachId, showCoachDetail.name); setShowCoachDetail(null); setBookingSuccess({ name: showCoachDetail.name, time: t }); }}>
      ...
      <AnimatePresence>
        {bookingSuccess && (
          <motion.div className="fixed inset-0 z-[1300] flex items-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
